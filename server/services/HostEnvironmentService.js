...
class HostEnvironmentService {
  ...
  async getHostCapabilities(forceRefresh = false) {
    ...
    for (const bin of this.KNOWN_BINARIES) {
      ...
      const isNative = this.NATIVE_EXECUTABLE_TOOLS.has(bin);

      const availability = isNative
      ...
      binaryMatrix[bin] = {
        ...
        nativeExecutionSupported: isNative && isInstalled,
    ...
    const capabilities = {
      ...
      nativeCapabilities: this.getNativeCapabilities(),
   ...
   * Authoritative native terminal capabilities descriptor (Step 2)

   * Exposes ONLY tools authorized in NATIVE_EXECUTABLE_TOOLS.
  ...
  getNativeCapabilities() {
    ...
    const metadata = {
      ...
      nmap: { label: 'Nmap Port Scanner', category: 'Network', description: 'Safe TCP port scanning and listening socket discovery', argumentProfile: '-sT -Pn -T4 -p 21,22,25,80,443,8080,8443 --host-timeout 8s {TARGET}' },

      dig: { label: 'DNS Dig Resolver', category: 'DNS & Network', description: 'Authoritative DNS record and zone lookup', argumentProfile: '+noall +answer +question {TARGET}' },

      curl: { label: 'HTTP Security Headers', category: 'Web Security', description: 'HTTP response header inspection and TLS protocol audit', argumentProfile: '-ILsS --connect-timeout 8 --max-time 10 {TARGET}' },

      whois: { label: 'WHOIS Registry Query', category: 'Reconnaissance', description: 'Domain registrar, ASN, and IP allocation intelligence', argumentProfile: '{TARGET}' },

      openssl: { label: 'SSL/TLS Certificate Audit', category: 'Web Security', description: 'TLS handshake, cipher suites, and certificate chain dossier', argumentProfile: 's_client -connect {HOST}:443 -servername {HOST} -brief' },

      ping: { label: 'ICMP Ping Reachability', category: 'Network', description: 'ICMP packet transmission and round-trip latency probe', argumentProfile: '-c 2 {TARGET}' },

      traceroute: { label: 'Network Route Traceroute', category: 'Network', description: 'Network gateway hop path and transit latency trace', argumentProfile: '-m 8 -q 1 -w 2 {TARGET}' }
    ...
    for (const bin of this.NATIVE_EXECUTABLE_TOOLS) {

      const resolved = this.resolveBinaryPath(bin);

      const isInstalled = Boolean(resolved);

      const meta = metadata[bin] || {};
      ...
      nativeList.push({
        ...
        alias: aliasMap[bin] || bin,

        label: meta.label || bin,
        ...
        argumentProfile: meta.argumentProfile || '{TARGET}',

        installed: isInstalled,

        availability: isInstalled ? 'AVAILABLE' : 'NOT INSTALLED'
