 ...
 * @description Storage abstraction wrapper for Assets.
 */
class AssetRepository extends IRepository {
    /**
     * @param {Object} deps
    ...
    constructor(deps) {
        ...
        if (!deps || !deps.storageProvider) {
            throw new Error('AssetRepository requires storageProvider');
        }
    ...
    async findMany(query = {}) {
        ...
        return docs.map(doc => new AssetDTO(doc));
    }
    ...
    async paginate(query, page = 1, limit = 10) {
        ...
        return {
            ...result,
            data: result.data.map(doc => new AssetDTO(doc))
        };
    }
}
