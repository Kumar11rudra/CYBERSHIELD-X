...
export default function ScanDetailPage() {
  ...
  const handleShareScan = async () => {
    try {
      const res = await api.patch(`/history/${id}/share`);
      setScan(prev => ({ ...prev, isPublic: res.data.isPublic }));
      if (res.data.isPublic) {
        const publicUrl = `${window.location.origin}/shared-scan/${id}`;
        await navigator.clipboard.writeText(publicUrl);
        toast.success('Scan is now public! Link copied to clipboard.');
      } else {
        toast.success('Scan is now private.');
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center gap-2 font-mono text-xs text-cyber-muted">
        <Link to="/history" className="hover:text-cyber-accent">History</Link>
        <span>/</span>
