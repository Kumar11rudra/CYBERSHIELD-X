...
const IntelligenceHUD = () => {
    ...
    return (
        <div className="font-mono text-[10px] text-cyber-accent uppercase tracking-[0.2em] mb-4 overflow-hidden h-4">
            ...
            >
                <p>» {t('breach.hud1') || 'INITIALIZING SECURE OSINT SCANNER'}</p>
                <p>» {t('breach.hud2') || 'QUERYING COMPROMISED IDENTITY INDEXES'}</p>
                <p>» {t('breach.hud3') || 'EVALUATING ZERO-KNOWLEDGE HASH REPOSITORIES'}</p>
                <p>» {t('breach.hud4') || 'COMPILING INCIDENT INTELLIGENCE DOSSIER'}</p>
            </motion.div>
        </div>
    );
};
...
const BreachCheckerPage = () => {
    ...
    const handleCheck = async (e) => {
        ...
        try {
            let endpoint = '/breach/email';
            let payload = { email: target };

            if (type === 'password') {
                endpoint = '/breach/password';
                payload = { password: target };
            } else if (type === 'phone') {
                endpoint = '/breach/phone';
                payload = { phone: target };
            }

            const res = await api.post(endpoint, payload);

            // Format password result into unified breach display schema
            let formattedData = res.data;
            if (type === 'password') {
                const count = res.data.count || res.data.breachCount || 0;
                const isBreached = res.data.compromised || res.data.breached || count > 0;
                formattedData = {
                    found: isBreached,
                    total: count,
                    source: res.data.source || 'HaveIBeenPwned k-Anonymity Engine',
                    methodology: 'SHA-1 k-Anonymity Range Query (NIST SP 800-63B)',
                    leaks: isBreached ? [{
                        id: 'PWN-001',
                        name: 'Compromised Password Hash Detected',
                        date: 'Identified in Global Breach Corpus',
                        description: `This password was detected ${count.toLocaleString()} times in compromised database dumps. Attackers use it in automated credential stuffing and dictionary attacks.`,
                        dataClasses: ['Password Hash', 'Plaintext Dictionary Match'],
                        severity: 'CRITICAL',
                        marketValue: 'High Exploitation Value',
                        hackerGroup: 'Global Credential Stuffing Bots',
                        threatActorType: 'Automated Botnet',
                        rationale: 'NIST SP 800-63B guidelines mandate immediately discarding compromised passwords.',
                        darkWebStatus: 'Public Exposure',
                        actorHistory: 'Active in wordlists and automated bruteforce dictionaries.',
                        sourceForum: 'Global Data Breach Dumps',
                        availability: 'Publicly Accessible Wordlists',
                        proof: 'k-Anonymity 5-char hash prefix match verified.',
                        recoveryPath: 'Immediately rotate this password across all services and enable Multi-Factor Authentication.'
                    }] : []
                };
            }
            ...
            setError(err.response?.data?.error || t('breach.error') || 'Breach check failed. Verify target input.');
            setLoading(false);
        }
    };
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    <div className="text-center mb-12">
                        ...
                        <p className="font-mono text-xs text-cyber-muted uppercase tracking-[0.3em] bg-white/5 py-2 px-6 inline-block rounded-full border border-white/10">
                            OSINT Intelligence & k-Anonymity Pool
                        </p>
                    </div>
                    ...
                    <div className="flex justify-center gap-3 mb-8 flex-wrap">
                        {[
                            { id: 'email', label: '📧 Email Scanner' },
                            { id: 'password', label: '🔑 Password Audit (k-Anonymity)' },
                            { id: 'phone', label: '📱 Phone Scanner' }
                        ].map((choice) => (
                            <button
                                key={choice.id}
                                onClick={() => {
                                ...
                                className={`px-6 py-3 rounded-xl font-mono text-xs transition-all border ${
                                    type === choice.id
                                    ? 'bg-cyber-accent/20 border-cyber-accent text-cyber-accent shadow-[0_0_15px_rgba(0,191,255,0.2)]'
                        ...
                        <div className="flex flex-col md:flex-row gap-4">
                            <div className="flex-1 relative">
                                <input
                                    ...
                                    placeholder={
                                        type === 'email'
                                            ...
                                            : type === 'password'
                                            ? 'Enter password to check k-Anonymity exposure...'
                                            : 'Enter international phone number (+1...)'
                                    }
                                    value={target}
                                ...
                                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-mono text-white/40">
                                    {type === 'password' ? '🔒 Zero-Knowledge 5-char SHA-1' : '🛡️ Encrypted Query'}
                                </div>
                            </div>
                            <button
                                type="submit"
                            ...
                            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}>
                                {result.found ? (
                                    <div className="space-y-8">
                                        <div className="flex items-center gap-6 border-b border-cyber-red/20 pb-8 relative flex-wrap">
                                            ...
                                            <div>
                                                <h2 className="text-3xl font-black text-cyber-red uppercase">Compromise Confirmed</h2>
                                                <p className="font-mono text-xs text-cyber-muted uppercase tracking-widest">{result.methodology || result.source}</p>
                                            </div>
                                            <button
                                                onClick={() => {
                                                    if (!user) {
                                                    ...
                                                    exportBreachPdf(result, type === 'password' ? '[CONFIDENTIAL_CREDENTIAL]' : target, user);
                                                }}
                                                disabled={exporting}
                                                ...
                                                >
                                                    ...
                                                    <div className="bg-black/80 rounded-lg p-4 border border-white/5 mb-6">
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                                            <div>
                                                                <p className="font-mono text-[9px] text-cyber-accent uppercase mb-1">» Threat Impact</p>
                                                        ...
                                                        {leak.darkWebStatus && (
                                                            <div className="bg-white/5 p-3 rounded border border-white/10 mb-4">
                                                                <div className="flex justify-between items-center mb-2">
                                                                    <p className="font-mono text-[9px] text-cyber-muted uppercase">» Exposure Pedigree</p>
                                                    ...
                                                    {leak.recoveryPath && (
                                                        <div className="mt-6 pt-6 border-t border-white/5">
                                                            <p className="text-[10px] font-bold text-cyber-accent mb-2">RECOVERY ACTION:</p>
                                        ...
                                        <div className="cyber-card p-6 mt-12 bg-cyber-accent/5 border-dashed">
                                            <h4 className="text-sm font-black text-cyber-accent mb-4 uppercase tracking-[0.2em]">Zero-Knowledge Intelligence Assurance</h4>
                                            <p className="text-xs text-cyber-muted leading-relaxed">
                                                Queries are evaluated using mathematical range hashing (NIST SP 800-63B). No plaintext passwords, account credentials, or identifying telemetry are permanently stored or transmitted in unencrypted form.
                                            </p>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="cyber-card p-12 text-center border-cyber-green/50 relative">
                                        ...
                                        <h3 className="text-2xl font-black text-cyber-green mb-2">Zero Compromise Identified</h3>
                                        <p className="text-cyber-muted text-sm max-w-md mx-auto">
                                            {result.message || 'Target identifier is clean across current synchronized dark web indices and password exposure repositories.'}
                                        </p>
                                    </div>
                                )}
                            </motion.div>
